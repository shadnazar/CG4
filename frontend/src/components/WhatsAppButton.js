...
function WhatsAppButton({ 
  ...
  if (position === 'inline') {
    return (
      <button
        onClick={handleClick}
        className={`flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white font-semibold py-3 px-6 rounded-xl transition-colors ${className}`}
  ...
  // Fixed floating button - simple icon only, no tooltip
  return (
    <button
      onClick={handleClick}
